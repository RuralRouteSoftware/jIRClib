...
public interface IRCConstants {
    ...
    /**
     * Format: "&lt;channel name&gt; You have joined too many channels". <br>
     ...
     * 412 - 414 are returned by PRIVMSG to indicate that the message wasn't delivered for some reason. ERR_NOTOPLEVEL
     * and ERR_WILDTOPLEVEL are errors that are returned when an invalid use of "PRIVMSG $&lt;server&gt;" or
     ...
     * Returned by the summon after a SUMMON command for a user was unable to be performed since they were not logged
     * in.
     */
     ...
     * Returned by the server to any link which tries to change part of the registered details (such as password or user
     ...
     * Returned to a client which attempts to register with a server which does not been setup to allow connections from
     * the host the attempted connection is tried.
     */
     ...
     * Returned after an attempt to connect and register yourself with a server which has been setup to explicitly deny
     * connections to you.
     */
    ...
    /**
     * Format: "Permission Denied- You're not an IRC operator". Any command requiring operator privileges to operate
     ...
     * Any command requiring 'chanop' privileges (such as MODE messages) must return this error if the client making the
     ...
     * Reply format used by USERHOST to list replies to the query list. The reply string is composed as follows:<br>
     ...
     * These replies are used with the AWAY command (if allowed). RPL_AWAY is sent to any client sending a PRIVMSG to a
     * client which is away. RPL_AWAY is only sent by the server to which the client is connected. Replies RPL_UNAWAY
     * and RPL_NOWAWAY are sent when the client removes and sets an AWAY message.
     */
    public static final int RPL_NOWAWAY = 306;

    /**
     * Format: "&lt;nick&gt; &lt;user&gt; &lt;host&gt; * :&lt;real name&gt;". <br>
     ...
     * query nick is found) or return an error reply. The '*' in RPL_WHOISUSER is there as the literal character and not
     * as a wild card. For each reply set, only RPL_WHOISCHANNELS may appear more than once (for long lists of channel
     * names). The '@' and '+' characters next to the channel name indicate whether a client is a channel operator or
     * has been granted permission to speak on a moderated channel. The RPL_ENDOFWHOIS reply is used to mark the end of
     ...
     * RPL_TOPIC is sent back else public static final int RPL_NOTOPIC.
     */
    ...
    /**
     * Format: "&lt;channel&gt; &lt;nick-who-set-topic&gt; &lt;seconds&gt;". The seconds are not milliseconds; just
     * multiply it with 1000 and then format a date with it.
     */
    ...
    /**
     * Format: "&lt;version&gt;.&lt;debuglevel&gt; &lt;server&gt; &lt;comments&gt;". <br>
    ...
    /**
     * Format: "&lt;mask&gt; &lt;server&gt; &lt;hopcount&gt; &lt;server info&gt;". <br>
     * In replying to the LINKS message, a server must send replies back using the RPL_LINKS numeric and mark the end of
     ...
     * RPL_BANLIST and RPL_ENDOFBANLIST messages. A separate RPL_BANLIST is sent for each active banid. After the banids
     * have been listed (or if none present) a RPL_ENDOFBANLIST must be sent.
     */
    ...
    /**
     * Format: "&lt;string&gt;". <br>
     * A server responding to an INFO message is required to send all its 'info' in a series of RPL_INFO messages with a
     * RPL_ENDOFINFO reply to indicate the end of the replies.
     */
     ...
     * If the USERS message is handled by a server, the replies RPL_USERSTART, RPL_USERS, RPL_ENDOFUSERS and RPL_NOUSERS
     * are used. RPL_USERSSTART must be sent first, following by either a sequence of RPL_USERS or a single RPL_NOUSER.
    ...
    /**
     * Format: "Link [version and debug level] [destination] [next server]". <br>
    ...
    /**
     * Format: "Serv &lt;class&gt; &lt;int&gt;S &lt;int&gt;C &lt;server&gt; &lt;nick!user|*!*&gt;@&lt;host|server&gt;". <br>
     * The RPL_TRACE* are all returned by the server in response to the TRACE message. How many are returned is
     * dependent on the the TRACE message and whether it was sent by an operator or not. There is no predefined order
     * for which occurs first. Replies RPL_TRACEUNKNOWN, RPL_TRACECONNECTING and RPL_TRACEHANDSHAKE are all used for
     * connections which have not been fully established and are either unknown, still attempting to connect or in the
    ...
    /**
     * Format: "&lt;linkname&gt; &lt;sendq&gt; &lt;sent messages&gt; &lt;sent bytes&gt; &lt;received messages&gt;
    ...
    /**
     * Format: "There are &lt;integer&gt; users and &lt;integer&gt; invisible on &lt;integer&gt; servers". <br>
    ...
    /**
     * Format: "I have &lt;integer&gt; clients and &lt;integer&gt; servers". <br>
     * In processing an LUSERS message, the server sends a set of replies from RPL_LUSERCLIENT, RPL_LUSEROP,
     * RPL_USERUNKNOWN, RPL_LUSERCHANNELS and RPL_LUSERME. When replying, a server must send back RPL_LUSERCLIENT and
     * RPL_LUSERME. The other replies are only sent back if a non-zero count is found for them.
     */
    ...
    /**
     * This is part of the mIRC code and shows that bold, underline and colors end. Here it is as the ASCII decimal int
     * 15.
     */
    public static final char COLOR_END_INDICATOR = 15; // ASCII code

    /**
     * This is part of the mIRC code and indicates that the client's colors are reversed (background -&gt; foreground
     * and foreground -&gt; background). Here it is as the ASCII decimal int 1.
     */
    public static final char COLOR_REVERSE_INDICATOR = 22; // ASCII code

    /**
     * This is part of the mIRC code and shows that a PRIVMSG is an ACTION (<code>/me</code>). Here it is as the ASCII
     * decimal int 22.
     */
