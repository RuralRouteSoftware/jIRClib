 ...
 *  &#42;/
 * IRCConnection conn = new IRCConnection(&quot;irc.somenetwork.com&quot;, 6667, 6669, null, &quot;Foo&quot;, &quot;Mr. Foobar&quot;,
 *         &quot;foo@bar.com&quot;);
 * 
...
public class IRCConnection extends Thread {
    ...
    /**
     * This is like a UNIX-runlevel. Its value indicates the level of the <code>IRCConnection</code> object.
     * <code>0</code> means that the object has not yet been connected, <code>1</code> means that it's connected but not
     * registered, <code>2</code> means that it's connected and registered but still waiting to receive the nickname the
     * first time, <code>3</code> means that it's connected and registered, and <code>-1</code> means that it was
     * connected but is disconnected. Therefore the defaultvalue is <code>0</code>.
     */
    ...
    /**
     * The <code>int[]</code> contains all ports to which we are going to try to connect. This can be a portrange from
     * port 6667 to 6669, for example.
     */
    protected int[] ports;
    ...
    /**
     * The <code>String</code> contains the name of the character encoding used to talk to the server. This can be
     * ISO-8859-1 or UTF-8 for example. The default is ISO-8859-1.
     */
    ...
    /**
     * This <code>int</code> is the connection's timeout in milliseconds. It's used in the
     * <code>Socket.setSoTimeout</code> method. The default is <code>1000 * 60 * 15</code> millis which are 15 minutes.
     */
     ...
     *             If the <code>host</code> or <code>ports</code> is <code>null</code> or <code>ports</code>' length is
     *             <code>0</code>.
     * @see #connect()
     */
    ...
    public IRCConnection(String host, int portMin, int portMax, String pass, String nick, String username,
            String realname) {
        this(host, portRangeToArray(portMin, portMax), pass, nick, username, realname);
    }
    ...
    /**
     * The <code>Thread</code> is started by the <code>connect</code> method. It's task is to receive strings from the
     * IRC server and hand them over to the <code>get</code> method.
     */
    public void run() {
        try {
            String line;
    ...
    /**
     * Sends a String to the server. You should use this method only, if you must do it. For most purposes, there are
     * <code>do*</code> methods (like <code>doJoin</code>). A carriage return line feed (<code>\r\n</code>) is appended
     * automatically.
     * 
     * @param line
     *            The line which should be send to the server without the trailing carriage return line feed (
     *            <code>\r\n</code>).
     */
     ...
     * 
     * @param pong
     *            <code>true</code> to enable automatic <code>PONG</code> reply, <code>false</code> makes the class fire
    ...
    /**
     * Changes the character encoding used to talk to the server. This can be ISO-8859-1 or UTF-8 for example. This
     * property must be set before a call to the <code>connect()</code> method.
     * 
     * @param encoding
     */
    ...
    /**
     * Returns the port to which the <code>IRCConnection</code> connected, or <code>0</code> if the connection failed or
     * wasn't tried yet.
     * 
     * @return The port to which the <code>IRCConnection</code>, or <code>0</code> if the connection failed or wasn't
     *         tried yet.
     */
    public int getPort() {
    ...
    /**
     * Generates a <code>String</code> with some information about the instance of <code>IRCConnection</code>. Its
     * format is: <code>
    ...
    public String toString() {
        return getClass().getName() + "[" + host + "," + getPort() + "," + username + "," + nick + ","
                + realname + "," + pass + "," + isConnected() + "]";
    }
     ...
     * 
     * @param ping
     *            The ping which was received in <code>onPing</code>. It's a <code>String</code>, because sometimes on
     *            some networks the server-hostname (for example splatterworld.quakenet.org) is given as parameter which
     *            would throw an Exception if we gave the ping as long.
     */
