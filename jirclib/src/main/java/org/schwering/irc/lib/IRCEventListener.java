 ...
 * <p>
 * The <code>IRCEventListener</code> is used by the <code>IRCConnection.addEventListener(IRCEventListener)</code> method
 * to add a listener which listens to the connection for incoming IRC events like <code>PRIVMSG</code>s or numeric
 * replies.
 * <p>
 * Supported events:
 * <ul>
...
public interface IRCEventListener extends EventListener, IRCConstants {
     ...
     * @param user
     *            The user who invites another. Contains nick, username and host.
     * @param passiveNick
     ...
     * @param user
     *            The user who kicks another user from a channel. Contains nick, username and host.
     * @param passiveNick
     *            The nickname of the user who is kicked from a channel (passive).
     * @param msg
     *            The message the active user has set. This is <code>""</code> if no message was set.
     */
    ...
    /**
     * Fired when an operator changes the modes of a channel. For example, he can set somebody as an operator, too, or
     * take him the oper-status. Also keys, moderated and other channelmodes are fired here.
     * 
     * @param chan
     *            The channel in which the modes are changed.
     * @param user
     *            The user who changes the modes. Contains nick, username and host.
     * @param modeParser
     *            The <code>IRCModeParser</code> object which contains the parsed information about the modes which are
     *            changed.
     */
    ...
    /**
     * Fired when somebody changes somebody's usermodes. Note that this event is not fired when a channel-mode is set,
     * for example when someone sets another user as operator or the mode moderated.
     * 
     * @param user
     *            The user who changes the modes of another user or himself. Contains nick, username and host.
     * @param passiveNick
     *            The nickname of the person whose modes are changed by another user or himself.
     * @param mode
     ...
     * @param user
     *            The user who notices another person or a group. Contains nick, username and host.
     * @param msg
     *            The message.
     */
     ...
     * @param user
     *            The user who parts from a channel. Contains nick, username and host.
     * @param msg
     *            The part-message which is optionally. If it's empty, msg is <code>""</code>.
     */
    ...
    /**
     * Fired when a <code>PING</code> comes in. The IRC server tests in different periods if the client is still there
     * by sending PING &lt;ping&gt;. The client must response PONG &lt;ping&gt;.
     * 
     * @param ping
     ...
     * @param user
     *            The user who sent the <code>PRIVMSG</code>. Contains nick, username and host.
     * @param msg
     ...
     * @param msg
     *            The optional message. <code>""</code> if no message is set by the user.
     */
    ...
    /**
     * Fired when the topic is changed by operators. Note that the topic is given as a numeric reply fired in
     * <code>onReply</code> when you join a channel.
     * 
     * @param chan
     *            The channel where the topic is changed.
     * @param user
     *            The user who changes the topic. Contains nick, username and host.
     * @param topic
     *            The new topic.
     */
     ...
     * @param command
     *            The command of the incoming line.
     * @param middle
     *            The part until the colon (<code>:</code>).
     * @param trailing
     *            The part behind the colon (<code>:</code>).
     */
