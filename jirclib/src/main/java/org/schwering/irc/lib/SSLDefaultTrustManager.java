 ...
 * <p>
 * In many cases you should change the <code>SSLIRCConnection</code>'s <code>TrustManager</code>. For examle if you
 * write an IRC client for human users, you may want to ask the user whether he accepts the server's certificate or not.
 * You could do this by a new class which extends the <code>SSLDefaultTrustManager</code> class and overrides the
 * <code>checkServerTrusted</code> method and asks the user whether he wants to accept the certification or not.
 * 
...
public class SSLDefaultTrustManager implements X509TrustManager {
    ...
    /**
     * Throws <code>CertificateException</code>. This method would check whether we (the server) trust the client. But
     * we are the client and not the server. <br />
    ...
    /**
     * Invoked when the client should check whether he trusts the server or not. This method trusts the server. But this
     * method can be overriden and then ask the user whether he truts the client or not.
     * 
     * @param chain
     *            The peer certificate chain.
     * @param authType
